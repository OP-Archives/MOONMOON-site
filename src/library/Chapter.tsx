    ...
    >
      <div className="w-full relative overflow-hidden rounded-t aspect-[400/530] bg-[#1a1a1b]">
        {chapter.image ? (
          ...
          <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-500 text-xs">
            No image
          </span>
        )}
      </div>
      <div className="px-1 py-0.5 text-center min-w-0 w-full">
        <CustomWidthTooltip title={chapter.name}>
          <span className="text-blue-500 font-medium block text-xs truncate">{chapter.name}</span>
        </CustomWidthTooltip>
        <span className="text-blue-500 text-xs">{chapter.count || 0} EPs</span>
      </div>
    </Link>
  );
});
